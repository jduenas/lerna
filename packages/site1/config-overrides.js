...
function expandRulesInclude(rules, include) {
  rules.forEach(function (rule) {
    ...
    if (rule.oneOf) expandRulesInclude(rule.oneOf, include)
  })
}
...
function expandPluginsScope(plugins, dirs, files) {
  ...
  const pluginPos = plugins.map((x) => x.constructor.name).indexOf(pluginName)
  if (pluginPos !== -1) {
...
function configPaths(configPath = '') {
  const confPath =
    configPath.length > 0 &&
    fs.existsSync(path.resolve(paths.appPath, configPath))
  ...
  if (!confPath)
    throw Error(
      'react-app-rewire-alias:configPaths: there is no config file found'
    )

  const conf = require(confPath)
  const extmsg = !conf.extends
    ? ''
    : `, specify ${conf.extends} instead of ${confPath} for configPaths()`

  if (typeof conf.compilerOptions.paths !== 'object')
    throw Error(`
      react-app-rewire-alias:configPaths: array expected for paths${extmsg}`)

  if (!conf.compilerOptions || !conf.compilerOptions.paths) return {}
